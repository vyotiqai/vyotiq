...
Verify with `pnpm typecheck`, `pnpm test`, `pnpm lint`. Launch app with
`pnpm start` (production preview) or `pnpm dev` (HMR). When launching from an
IDE-integrated shell, unset `ELECTRON_RUN_AS_NODE` first
(`Remove-Item Env:ELECTRON_RUN_AS_NODE`) — the IDE exports `1`, which makes
Electron boot as plain Node and crash with
`Cannot read properties of undefined (reading 'isPackaged')`. Killed dev
shells can orphan `electron.exe` processes holding the Vite port; stop them
before relaunching or the new window renders blank.
